...
SKANF classifies a CALL as a vulnerability only when:
  - The CALL target is one of these known DeFi token addresses, AND/OR
  - The function selector embedded in the calldata is one of the three
    ERC-20 transfer functions (transfer / approve / transferFrom)
  - AND at least one of: target contract, function selector, destination
    address, or token amount is attacker-controlled (calldata-tainted).
...
# Full set used by SKANF (with and without 0x prefix for matching convenience)
SENSITIVE_SIGNATURES: frozenset = frozenset({
    ...
    f"0x{ERC20_TRANSFER}",
    f"0x{ERC20_APPROVE}",
    f"0x{ERC20_TRANSFER_FROM}",
})

# ── Known DeFi token addresses — ETH mainnet ───────────────────────────────
# Source: SKANF /opt/skanf/constants/address.json (ETH list)
# Includes: USDT, BNB, USDC, stETH, WBTC, LINK, BUSD, SHIB, TON, wstETH,
#           USDS, ?, WETH, ?, USDe, weETH, PEPE, cbBTC, UNI, DAI, ...
...
SENSITIVE_ADDRESSES_ETH: frozenset = frozenset({
    addr.lower() for addr in [
        ...
        "0x21c2c96Dbfa137E23946143c71AC8330F9B44001",  # (unlabeled)
        "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2",  # WETH
        "0x54D2252757e1672EEaD234D27B1270728fF90581",  # (unlabeled)
        ...
        "0x85F17Cf997934a597031b2E18a9aB6ebD4B9f6a4",  # (unlabeled)
        "0xfAbA6f8e4a5E8Ab82F62fe7C39859FA577269BE3",  # (unlabeled)
        "0x418708dD507A2F0Cac24d31c60B350315F4C8009",  # (unlabeled)
        "0xE66747a101bFF2dBA3697199DCcE5b743b454759",  # GT
        "0xa3931d71877C0E7a3148CB7Eb4463524FEc27fbD",  # (unlabeled)
        "0xBc65ad17c5C0a2A4D159fa5a503f4992c7B545FE",  # (unlabeled)
        "0x7Fc66500c84A76Ad7e9c93437bFc5Ac33E2DDaE9",  # AAVE
        "0x4da27a545c0c5B758a6BA100e3a049001de870f5",  # stkAAVE
        "0x7712c34205737192402172409a8F7ccef8aA2AEc",  # (unlabeled)
        "0x6a9DA2D710BB9B700acde7Cb81F10F1fF8C89041",  # (unlabeled)
        "0xA0b73E1Ff0B80914AB6fe0444E65848C4C34450b",  # CRO
        "0x3c3a81e81dc49A522A592e7622A7E711c06bf354",  # (unlabeled)
        "0x6De037ef9aD2725EB40118Bb1702EBb27e4Aeb24",  # (unlabeled)
        "0xD850942eF8811f2A866692A623011bDE52a462C1",  # VEN
        "0x455e53CBB86018Ac2B8092FdCd39d8444aFFC3F6",  # (unlabeled)
        "0x9D39A5DE30e57443BfF2A8307A4256c8797A3497",  # (unlabeled)
        "0xaea46A60368A7bD060eec7DF8CBa43b7EF41Ad85",  # (unlabeled)
        "0x57e114B691Db790C35207b2e685D4A43181e6061",  # (unlabeled)
        "0x8236a87084f8B84306f72007F36F2618A5634494",  # (unlabeled)
        "0x6e1A19F235bE7ED8E3369eF73b196C07257494DE",  # (unlabeled)
        "0x8D983cb9388EaC77af0474fA441C4815500Cb7BB",  # USDG
        "0x519ddEff5d142Fc177d95F24952eF3D2EDe530BC",  # (unlabeled)
        "0xB50721BCf8d664c30412Cfbc6cf7a15145234ad1",  # ARB
        "0x163f8C2467924be0ae7B5347228CABF260318753",  # WLD
        "0xc5f0f7b66764F6ec8C8Dff7BA683102295E16409",  # FDUSD
        "0x4aeF9BD3fBb09d8f374436D9ec25711A1Be9BaCb",  # (unlabeled)
        "0xF5e11df1ebCf78b6b6D26E04FF19cD786a1e81dC",  # (unlabeled)
        "0xf34960d9d60be18cC1D5Afc1A6F012A723a28811",  # KCS
        "0x9f8F72aA9304c8B593d555F12eF6589cC3A579A2",  # MKR
    ]
})

# BSC mainnet equivalents (for multi-chain analysis)
...
# Combined set for network-agnostic checks
...
def is_sensitive_call(
    ...
    """
    Return True if this CALL matches SKANF's sensitivity criteria.

    A call is sensitive if:
      - target_addr is in the known DeFi token list for the network, OR
      - function_selector matches a known ERC-20 transfer function

    Either condition alone is sufficient; SKANF flags both.

    Args:
        target_addr:       Hex address of the CALL target (with or without 0x).
        function_selector: 4-byte hex selector (with or without 0x, 8 hex chars).
        network:           "ethereum", "bsc", or "all" for cross-chain.
    """
    if target_addr:
...
def erc20_calldata(selector_hex: str, attacker_addr: str) -> str:
    ...
    For transfer(attacker, max) and approve(attacker, max):
      selector(4) + attacker_padded(32) + max_uint256(32)

    For transferFrom(victim, attacker, max):
      selector(4) + zero_victim(32) + attacker_padded(32) + max_uint256(32)

    This matches SKANF's aeg.py calldata construction exactly.

    Args:
        selector_hex:  8-char hex function selector (no 0x prefix).
        attacker_addr: Hex address of the attacker/recipient (with 0x).

    Returns:
        Full calldata as 0x-prefixed hex string.
    """
    _WORD = 32
    ...
    if selector_hex.lstrip("0x").lower() == ERC20_TRANSFER_FROM:
        # transferFrom(from=zero, to=attacker, amount=max)
        # SKANF uses zero for the 'from' address (victim slot) since the
        # contract itself is the token holder being drained.
        zero = (0).to_bytes(_WORD, "big")
        payload = sel + zero + addr_bytes + MAX_UINT256
    else:
        # transfer(to=attacker, amount=max)  OR  approve(spender=attacker, amount=max)
